...
export function placeAnchored(
  ...
  const {
    side: preferred = 'block-end',
    offset = 0,
    arrowSize = 0,
    viewportPadding = WINDOW_PADDING,
    arrowPadding = 0,
  } = options
  ...
  const roomAfter =
    main.viewport - viewportPadding - (main.anchorStart + main.anchorSize) - gap

  const wantsBefore =
    preferred === 'block-start' || preferred === 'inline-start'
  ...
  const mainSize = maxSize ?? main.size
  ...
  if (centred < lowest) crossPos = lowest
  else if (centred > highest) crossPos = highest
