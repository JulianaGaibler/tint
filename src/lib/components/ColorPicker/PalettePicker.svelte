...
.palette
  ...
  min-height: 0
  // Pull the pane out of the popover's horizontal padding so the listbox
  // separator and row hover/selection backgrounds extend edge to edge. The
  // search row restores its own gutter so it stays visually aligned with
  // the tab strip above.
...
.listbox
  ...
  display: flex
  flex-direction: column
  // Padding-top is 0 so sticky category headers pin flush against the
  // listbox's top edge instead of leaving a 4px gap where the row above
...
.row
  ...
  // height regardless of leaf string content.
