...
.slider-fill
  ...
  // Progress bar envelopes the thumb
  // Simplified: (edge-padding + thumb-size + edge-padding) + (fill% * (100% - total-padding) / 100)
  width: min(calc(var(--edge-padding) + var(--thumb-size) + var(--edge-padding) + var(--fill-percentage) * (100% - var(--edge-padding) - var(--thumb-size) - var(--edge-padding)) / 100), 100%)
...
.slider
  ...
  // Visual states for fake thumb
  &:hover + .slider-track ~ .slider-thumb
