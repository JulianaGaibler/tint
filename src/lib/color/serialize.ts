...
export function toHex(c: Color): string {
  ...
  const clip = (v: number) => Math.max(0, Math.min(255, Math.round(v * 255)))
...
function toLegacyRgb(c: Color): string {
...
function toModernRgb(c: Color): string {
  const r = formatByte(c.components[0])
  const g = formatByte(c.components[1])
  const b = formatByte(c.components[2])
  const alpha = c.alpha < 1 ? ` / ${formatAlpha(c.alpha)}` : ''
...
function toLegacyHsl(c: Color): string {
...
function toColorFn(space: ColorSpace, c: Color): string {
