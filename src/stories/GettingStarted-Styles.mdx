...
- Font family tokens (`--tint-font-sans`, `--tint-font-serif`,
  `--tint-font-mono`). See [Fonts](/?path=/docs/getting-started-fonts--docs) for
...
can reference them directly without SASS or any import:
...
`full-bootstrap()`), so you can retheme them at runtime, with no SASS or rebuild
