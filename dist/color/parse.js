...
export function parseColor(input) {
    ...
    if (!trimmed)
        throw new ColorParseError(input, 'empty input');
    ...
    if (!fn)
        throw new ColorParseError(input, 'not a recognized color syntax');
    switch (fn.name) {
        case 'rgb':
        case 'rgba':
...
// ---------- hex ----------
function parseHex(input, trimmed) {
    const body = trimmed.slice(1);
    let r, g, b, a = 1;
    const expand = (h) => parseInt(h + h, 16) / 255;
    const byte = (h) => parseInt(h, 16) / 255;
    if (body.length === 3) {
        r = expand(body[0]);
        g = expand(body[1]);
        b = expand(body[2]);
        ...
        throw new ColorParseError(input, `hex must be 3, 4, 6, or 8 digits, got ${body.length}`);
    }
...
function parseNumber(token, ctx) {
    if (token === NONE_TOKEN)
        return { value: NaN, missing: true };
    const num = Number(token);
    if (!Number.isFinite(num)) {
        throw new ColorParseError(ctx.input, `expected number for ${ctx.channel}, got "${token}"`);
    }
    return { value: num, missing: false };
}
function parsePercent(token, scale, ctx) {
    if (token === NONE_TOKEN)
        return { value: NaN, missing: true };
    if (!token.endsWith('%')) {
        throw new ColorParseError(ctx.input, `expected percent for ${ctx.channel}, got "${token}"`);
    }
    const num = Number(token.slice(0, -1));
    if (!Number.isFinite(num)) {
        throw new ColorParseError(ctx.input, `bad percent for ${ctx.channel}: "${token}"`);
    }
    return { value: (num / 100) * scale, missing: false };
}
function parseNumberOrPercent(token, scale, ctx) {
    if (token.endsWith('%'))
        return parsePercent(token, scale, ctx);
    return parseNumber(token, ctx);
}
function parseHue(token, ctx) {
    if (token === NONE_TOKEN)
        return { value: NaN, missing: true };
    let value;
    if (token.endsWith('deg'))
        value = Number(token.slice(0, -3));
    else if (token.endsWith('rad'))
        value = (Number(token.slice(0, -3)) * 180) / Math.PI;
    else if (token.endsWith('grad'))
        value = Number(token.slice(0, -4)) * 0.9;
    else if (token.endsWith('turn'))
        value = Number(token.slice(0, -4)) * 360;
    else
        value = Number(token);
...
function parseAlpha(token, input) {
    if (token == null)
        return { alpha: 1, missing: false };
    const t = token.trim();
    if (t === NONE_TOKEN)
        return { alpha: NaN, missing: true };
    if (t.endsWith('%')) {
...
// ---------- rgb() ----------
function parseRgb(input, body) {
    const { channels, alpha } = splitChannels(body);
    if (channels.length !== 3) {
        throw new ColorParseError(input, `rgb() needs 3 channels, got ${channels.length}`);
    }
    const ctx = (channel) => ({ input, channel });
    ...
    const components = [r.value / 255, g.value / 255, b.value / 255];
...
// ---------- hsl() ----------
function parseHsl(input, body) {
    const { channels, alpha } = splitChannels(body);
    if (channels.length !== 3) {
        throw new ColorParseError(input, `hsl() needs 3 channels, got ${channels.length}`);
    }
    const ctx = (channel) => ({ input, channel });
...
// ---------- oklch() ----------
function parseOklch(input, body) {
    const { channels, alpha } = splitChannels(body);
    if (channels.length !== 3) {
...
// ---------- oklab() ----------
function parseOklab(input, body) {
    const { channels, alpha } = splitChannels(body);
    if (channels.length !== 3) {
...
function parseColorFn(input, body) {
    const { channels, alpha } = splitChannels(body);
