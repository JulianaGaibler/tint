...
export function orthogonalToPolar([l, a, b], eps = POLAR_EPSILON) {
    const chroma = Math.sqrt(a * a + b * b);
    let hue;
    if (Math.abs(a) < eps && Math.abs(b) < eps)
        hue = NaN;
    else if (Math.abs(chroma) < eps)
        hue = NaN;
    else
        hue = normalizeHue((Math.atan2(b, a) * 180) / Math.PI);
    return [l, chroma, hue];
}
export function polarToOrthogonal([l, c, h]) {
    if (Number.isNaN(h))
        return [l, 0, 0];
...
function toXyzD65(c) {
    switch (c.space) {
        ...
        case 'oklab': {
            ...
            const cubed = [lms[0] ** 3, lms[1] ** 3, lms[2] ** 3];
...
export function convert(c, target) {
    if (c.space === target)
        return c;
