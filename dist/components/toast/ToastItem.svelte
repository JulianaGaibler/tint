<script lang="ts">
  ...
  import IconCloseSmall from '../../icons/14-close.svg?raw'
...
<style>.toast {
  ...
  color: var(--tint-text);
  box-shadow: var(--tint-card-shadow, 0 4px 12px rgba(0, 0, 0, 0.1));
...
.toast[data-expanded=true][data-is-top=false]::after {
  inset-block-end: 100%;
}
.toast[data-expanded=true][data-is-top=true]::after {
...
.toast[data-mounted=true]:not([data-visible=true]):not([data-expanded=true]) {
  opacity: 0;
  pointer-events: none;
}
.toast[data-mounted=true]:not([data-visible=true]):not([data-expanded=true]) > * {
...
.toast[data-expanded=true][data-mounted=true] {
  transform: translateY(calc(var(--lift) * var(--offset))) scale(1) translateX(var(--swipe-x, 0px));
  height: var(--initial-height);
  opacity: 1;
}
.toast:not([data-front=true]):not([data-expanded=true])[data-mounted=true] {
...
.toast[data-swipe-out=right] {
  animation: swipe-out-right 200ms ease-out forwards;
}
.toast[data-swipe-out=left] {
  animation: swipe-out-left 200ms ease-out forwards;
}
.toast[data-removing=true]:not([data-swipe-out]):is([data-front=true], [data-expanded=true]) {
  transform: translateY(calc(var(--lift) * -100%)) scale(1) translateX(var(--swipe-x, 0px));
  opacity: 0;
}
.toast[data-removing=true]:not([data-swipe-out]):not([data-front=true]):not([data-expanded=true]) {
