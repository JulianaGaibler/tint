...
import { maxChromaIn } from '../../color/gamut';
import { CHROMA_MAX } from '../../color/constants';
import { contrast as wcagContrast, parseColor, ColorParseError, makeColor, } from '../../color';
/**
...
function sampleD(_h, _gamut, factor) {
    let d = '';
 ...
 */
function lruGet(cache, key, build, cap) {
 ...
 */
export function sampleGamutBoundary(hue, gamut = 'srgb') {
    const h = Math.round(hue * 2) / 2; // 0.5° bucket
    const key = `${h}|${gamut}`;
    const d = lruGet(BOUNDARY_CACHE, key, () => sampleD(h, gamut, (l) => maxChromaIn(l, h, gamut)), BOUNDARY_CAP);
    return {
        id: `boundary-${gamut}`,
        d,
        style: 'boundary',
 ...
 * engaged.
 */
export function sampleRelativeChromaCurve(hue, pct, gamut = 'srgb') {
    const h = Math.round(hue * 2) / 2;
    const p = Math.max(0, Math.min(100, Math.round(pct)));
    const key = `${h}|${p}|${gamut}`;
    const d = lruGet(ISO_CACHE, key, () => sampleD(h, gamut, (l) => (p / 100) * maxChromaIn(l, h, gamut)), ISO_CAP);
    return {
        id: 'iso-chroma',
        d,
        style: 'iso',
 ...
 */
export function sampleContrastCurve(hue, againstHex, targetRatio, space = 'oklch', gamut = 'srgb') {
    const h = Math.round(hue * 2) / 2;
    ...
    const ratioAt = (axis, lightnessFrac) => {
        var _a;
        const result = wcagContrast({
            color: makeAt(axis, lightnessFrac),
            against,
        });
        return (_a = result.ratio) !== null && _a !== void 0 ? _a : 1;
    };
    ...
    const ranges = new Array(CONTRAST_GRID + 1);
    const corners = new Array(CONTRAST_GRID + 1);
    for (let li = 0; li <= CONTRAST_GRID; li++) {
        const lf = li / CONTRAST_GRID;
        const range = axisRange(lf);
        ranges[li] = range;
        const row = new Array(CONTRAST_GRID + 1);
    ...
    for (let li = 0; li < CONTRAST_GRID; li++) {
        ...
        for (let ai = 0; ai < CONTRAST_GRID; ai++) {
            ...
            switch (code) {
                ...
                case 14:
                    if (ptB && ptL)
                        segments.push([ptB, ptL]);
                    break;
                case 2:
                case 13:
                    if (ptB && ptR)
                        segments.push([ptB, ptR]);
                    break;
                case 4:
                case 11:
                    if (ptT && ptR)
                        segments.push([ptT, ptR]);
                    break;
                case 8:
                case 7:
                    if (ptT && ptL)
                        segments.push([ptT, ptL]);
                    break;
                // Two adjacent corners above.
                case 3:
                case 12:
                    if (ptL && ptR)
                        segments.push([ptL, ptR]);
                    break;
                case 6:
                case 9:
                    if (ptB && ptT)
                        segments.push([ptB, ptT]);
                    break;
