import { MenuBehavior, WINDOW_PADDING, TOP_MENU_OFFSET, LEFT_MENU_OFFSET, } from './types';
/**
 ...
 */
export function checkIfInTriangle(a, b, c, s) {
    const as_x = s.x - a.x;
    const as_y = s.y - a.y;
 ...
 * logic
 */
export function calculatePosition(depth, parentItemRect, menuRect, behavior, windowDimensions, relativeDistance) {
    const coords = {
        x: 0,
        y: 0,
        endAlign: true,
    ...
    if (behavior === MenuBehavior.SELECT && relativeDistance) {
        ...
        const spaceBelow = windowDimensions.innerHeight -
            WINDOW_PADDING -
